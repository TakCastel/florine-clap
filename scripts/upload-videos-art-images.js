...
/**
 * Script pour uploader toutes les images de temp-videos-art-images dans Directus
 * et les associer aux contenus videos_art correspondants
 */
...
const DATA_FILE = path.join(projectRoot, 'videos-art-data.json')
  ...
  if (existing) {
    ...
    if (existing.folder !== folderId) {
      try {
        ...
        console.log(`  ✅ Déplacé dans le dossier "Vidéos/art"`)
      } catch (error) {
...
async function updateVideoArtImage(slug, imageId) {
  try {
    ...
    if (!response.data.data || response.data.data.length === 0) {
      console.warn(`⚠️  Item videos_art non trouvé pour le slug: ${slug}`)
      return false
    }
    ...
    await axiosInstance.patch(`/items/videos_art/${item.id}`, {
      image: imageId
    })
...
async function main() {
  ...
  console.log('📤 Upload des images videos-art dans Directus...\n')
  
  // 1. Obtenir ou créer le dossier "Vidéos/art"
  const folderId = await getOrCreateFolder('Vidéos/art')
  console.log(`📁 Dossier "Vidéos/art" (ID: ${folderId})\n`)
  ...
  const videosArtData = JSON.parse(dataContent)
  
  // 3. Créer un mapping image filename -> slug
  const imageToSlugMap = new Map()
  for (const item of videosArtData) {
  ...
  console.log(`📋 ${imageFiles.length} image(s) trouvée(s) dans temp-videos-art-images\n`)
  
  // 5. Uploader chaque image et l'associer au contenu correspondant
  let uploaded = 0
  let associated = 0
  let skipped = 0
  ...
  for (const imageFile of imageFiles) {
    ...
    if (slug) {
      const success = await updateVideoArtImage(slug, imageId)
      if (success) {
        associated++
      }
    } else {
      console.log(`  ⚠️  Aucun contenu videos_art trouvé pour cette image`)
    }
  }
  ...
  console.log(`   - ${associated} image(s) associée(s) à des contenus`)
  console.log(`   - ${skipped} image(s) ignorée(s)`)
  console.log(`\n💡 Vérifiez dans Directus que les images sont bien associées aux contenus videos_art`)
}
