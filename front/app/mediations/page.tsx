...
import { getAllMediations, Mediation, getHomeSettings, getPageBySlug } from '@/lib/directus'
...
const DEFAULT_SEO_TITLE = 'Médiation et Formation'
const DEFAULT_SEO_DESCRIPTION = 'Depuis une dizaine d\'années, en parallèle de mes projets artistiques, je propose des actions de médiation et des ateliers vidéo de réalisation, destinés principalement aux adolescent·es, étudiant·es et jeunes adultes, dans le cadre de dispositifs tels que Collège au cinéma, ou pour des écoles et des conservatoires.'

export async function generateMetadata() {
  const page = await getPageBySlug('mediations').catch(() => null)
  return buildMetadata({
    title: `Médiations artistiques - ${page?.seo_title || DEFAULT_SEO_TITLE}`,
...
async function MediationsContent() {
  const [mediations, homeSettings, page] = await Promise.all([getMediations(), getHomeSettings(), getPageBySlug('mediations').catch(() => null)])
  const heroImageUrl = page?.hero_image || homeSettings?.category_mediations_image || null
  ...
  return (
    <ContentListPage
      ...
      basePath="/mediations"
      title={page?.title || 'Médiations artistiques'}
