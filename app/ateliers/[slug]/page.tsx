    ...
    <div className="min-h-screen bg-white">
      <div className="mx-auto max-w-6xl px-4 py-12">
        <Breadcrumb 
          items={[
            { label: 'Accueil', href: '/' },
            { label: 'Médiations', href: '/ateliers' },
            { label: doc.title }
          ]}
          variant="grey"
        />
        <h1 className="text-4xl md:text-6xl lg:text-8xl font-montserrat font-bold tracking-wide text-theme-blue">{doc.title}</h1>
        <p className="mt-2 text-sm text-gray-600">
          {new Date(doc.date).toLocaleDateString('fr-FR')} · {doc.lieu}{doc.duree ? ` · ${doc.duree}` : ''}
        </p>
        {doc.modalites && (
          <div className="mt-6">
            <h3 className="text-lg font-semibold mb-2 text-theme-blue">Modalités</h3>
        ...
        {doc.lien_inscription && (
          <div className="mt-6">
            <a 
              ...
              className="inline-block bg-theme-blue text-white px-6 py-3 hover:bg-blue-700 transition-colors"
            >
              S'inscrire
            </a>
          </div>
